...
class TestProgram(unittest.TestCase):
    ...
    def run_test(self, program, test_name, use_args=False):
        ...
        # Prepare the command with the full path to the Python script
        cmd = ['python3', os.path.join(self.prog_dir, f'{program}.py')]
    
        # If use_args is True, the arguments are read from the .in file and passed to the command
        if use_args:
            with open(input_file, 'r') as f:
                args = f.read().strip().split()
            cmd.extend(args)  # Add the arguments from the .in file to the command
            process = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        else:
            # If use_args is False, the content of the .in file is passed as STDIN to the script
            with open(input_file, 'rb') as f:
                process = subprocess.run(cmd, stdin=f, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    
        # Check if the process exited with a non-zero status and output any errors
